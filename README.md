...
このアプリを動かすために必要なものを、先に一覧で整理します。

| マテリアル | どこで使うか | 役割 |
| --- | --- | --- |
| GitHubアカウント | GitHub | このリポジトリ、GitHub Actions、GitHub Pagesを管理します |
| GitHubリポジトリ | GitHub | Pythonコード、GASコード、README、生成された `docs` を置きます |
| GitHub Actions `daily_ai_news` | GitHub | ニュース生成、HTML生成、LINE送信、`docs` のcommit/pushを実行します |
| GitHub Secrets | GitHub | OpenAI APIキー、LINEトークン、送信先、承認済みユーザーAPIなどの秘密情報を保存します |
| GitHub Pages | GitHub | `main` ブランチの `/docs` フォルダを詳細版HTMLとして公開します |
| `docs` フォルダ | GitHubリポジトリ | `YYYY-MM-DD.html`、`latest.html`、送信済みマーカーを保存します |
| Googleスプレッドシート | Google | LINE登録申請者の `userId`、表示名、`pending / approved / rejected` を管理します |
| Google Apps Script | Google | LINE Webhook受信、承認管理、承認済み一覧API、GitHub Actions起動を担当します |
| GAS WebアプリURL | Google Apps Script | LINE DevelopersのWebhook URL、GitHub Actions側の `APPROVED_USERS_ENDPOINT` として使います |
| GAS時間主導トリガー | Google Apps Script | 毎朝7:07 JSTごろと7:37 JSTごろにGitHub Actionsを起動します |
| LINE公式アカウント | LINE | 友だち追加、登録申請、管理者コマンド、AIニュース受信に使います |
| LINE Messaging APIチャネル | LINE Developers | LINEへのpush message、プロフィール取得、Webhook署名検証に使います |
| OpenAI APIキー | OpenAI Platform | Responses APIと `web_search` toolでニュース収集・要約を行います |
| OpenAI Billing残高 | OpenAI Platform | API実行に必要です。残高不足だとニュース生成が失敗します |
| GitHub dispatch用token | GitHub | GASからGitHub Actionsの `workflow_dispatch` を呼ぶために使います |

## 簡易構成

このアプリには、大きく分けて2つの流れがあります。

1つ目は、友だち追加と承認の流れです。

```text
友人がLINE公式アカウントを友だち追加
→ LINEがWebhookをGAS WebアプリURLへ送る
→ GASの doPost(e) が userId と表示名を取得
→ Googleスプレッドシートへ pending として保存
→ 管理者LINEへ登録申請通知
→ 管理者がLINEで approve <userId> を送る
→ GASがスプレッドシートのstatusを approved に変更
→ 承認された人へLINEで承認通知
```

2つ目は、毎朝ニュースを作って送る流れです。

```text
GAS時間主導トリガーが7:07 JSTごろに発火
→ GASの dispatchDailyAiNewsWorkflow() がGitHub APIを呼ぶ
→ GitHub Actions daily_ai_news が workflow_dispatch で起動
→ main.py がGitHub Secretsから設定を読み込む
→ approved_users.py がGASのapproved一覧APIから送信先を取得
→ news_briefing.py がOpenAI Responses API + web_searchでニュース生成
→ site_generator.py が docs/YYYY-MM-DD.html と docs/latest.html を生成
→ line_sender.py がLINE Messaging APIで承認済みユーザーへ送信
→ 成功時は docs/.daily_ai_news_sent_YYYY-MM-DD を作成
→ GitHub Actionsが docs の変更をcommit/push
→ GitHub Pagesで詳細版HTMLが公開される
```

7:07 JSTごろの実行が失敗した場合は、7:37 JSTごろのバックアップトリガーが同じ流れを再実行します。すでに送信済みマーカーがある日は、バックアップ側ではニュース生成もLINE送信も行わず終了します。

主なエンドポイントと、そこで起きることは次です。

| エンドポイント | 呼ぶ側 | 受ける側 | 起きること |
| --- | --- | --- | --- |
| LINE Webhook URL | LINE Developers | GAS Webアプリの `doPost(e)` | 友だち追加、通常メッセージ、`approve` などの管理者コマンドを受けます |
| 承認済みユーザー一覧API | GitHub Actions | GAS Webアプリの `doGet(e)` | `?action=approved&key=...` で `approved` ユーザー一覧をJSONで返します |
| GitHub Actions dispatch API | GAS | GitHub API | `workflow_dispatch` で `daily_ai_news` を起動し、`scheduled_dispatch=true` を渡します |
| GitHub Pages URL | LINEを開いた人 | GitHub Pages | `docs/latest.html` や `docs/YYYY-MM-DD.html` の詳細版HTMLを表示します |
| LINE Messaging API push endpoint | Python / GAS | LINE | Pythonはニュース配信、GASは登録申請通知・承認通知・エラー通知を送ります |

承認済みユーザー一覧APIは、`APPROVED_USERS_API_KEY` が一致した場合だけJSONを返します。キーが違う場合は拒否します。LINE Webhookは `LINE_CHANNEL_SECRET` で署名検証を行い、管理者コマンドは `ADMIN_LINE_USER_ID` と一致するユーザーだけ実行できます。

最終的に残るものは次です。

- LINEには、スマホで読みやすい短縮版ニュースが届きます。
- GitHubの `docs/YYYY-MM-DD.html` に、その日の詳細版HTMLが残ります。
- GitHubの `docs/latest.html` は最新の詳細版へ更新されます。
- GitHub Pagesで詳細版URLが開けるようになります。
- Googleスプレッドシートには、登録者の承認状態が残ります。
- GitHub Actionsログには、OpenAI生成、HTML生成、LINE送信、commit/pushの結果が残ります。
- 成功した定期実行では、`docs/.daily_ai_news_sent_YYYY-MM-DD` が送信済みマーカーとして残ります。
...
- LINE短縮版と詳細版で、カテゴリ別ニュースの内容・順序・重要度・信頼度・URLがずれないように、`news_items` という共通リストを正本にします。
...
現在のプロンプトでは、Apple、Google、Microsoft、Meta、Amazon、OpenAI、Anthropic、NVIDIAは「対象企業例」です。この8社はデフォルトでは同じ重要度の対象企業として扱いますが、対象はこの8社だけに限定しません。AI・IT技術全体で重要度が高いニュースを優先します。

表示順は `Apple → Google → Microsoft → Meta / Amazon → Other AI` に固定しています。ただし、ニュースの採用判断は表示順ではなく、ユーザー影響、プラットフォーム重要度、新しさ、信頼性、実用性、技術的な面白さを基準にします。
...
LINEメッセージでは、スマホで読みやすいように各カテゴリの前に `--------------------` を入れています。重要度と信頼度は `注目：高 / 公式` のように1行へまとめ、空カテゴリは `該当なし（24時間以内の主要ニュースなし）` と短く表示します。

LINEメッセージが長い場合、`line_sender.py` が複数メッセージに分割します。分割時も本文を落とさず、最初の1通には詳細版リンクを追加します。詳細版リンクは重複しないよう、原則として最初の1通だけに入ります。
...
詳細版はカテゴリ別に整理します。ただし、LINE短縮版と詳細版で、同じニュースのタイトル、重要度、信頼度、URLが変わらないように、内部では `news_items` という共通リストを正本にしています。カテゴリ順は `Apple → Google → Microsoft → Meta / Amazon → Other AI` に固定し、カテゴリ内では `news_items` の相対順序を保ちます。

詳細版の各ニュース見出し番号はカテゴリ内の連番です。LINE短縮版も同じカテゴリ順・カテゴリ内番号で出すため、LINEと詳細版を見比べても同じニュースを追えるようにしています。
...
GPTへ渡しているプロンプトの本体は、次の場所です。

```text
news_briefing.py
→ _build_prompt(date_jst, detail_url)
```

現在のプロンプトでは、次のようになっています。

- `対象企業例:` として Apple、Google、Microsoft、Meta、Amazon、OpenAI、Anthropic、NVIDIAを挙げています。
- この8社は同じ重要度の対象企業例ですが、ニュース収集対象をこの8社だけに限定していません。
- LINE版と詳細版の順番は `Apple → Google → Microsoft → Meta / Amazon → Other AI → 今日のMust Read → 深掘り候補 → 全体所感` です。
- LINE版はスマホで読みやすいように、区切り線、短い空カテゴリ表示、`注目：重要度 / 信頼度` の1行表示を使います。
- 詳細版は `AIニュース詳細ブリーフィング` という見出しで、`Selection basis / 選定基準` から始まります。

ただし、プロンプトを書き換えるだけでは、すべての表示が変わるとは限りません。Structured Outputsと最終表示はPython側でも整形しています。

| 変更したいもの | 主な編集箇所 |
| --- | --- |
| GPTへの指示文 | `news_briefing.py` の `_build_prompt()` |
| JSONの必須項目やカテゴリ候補 | `news_briefing.py` の `BRIEFING_SCHEMA` |
| LINEに最終的に届く文面 | `news_briefing.py` の `_render_line_message()` |
| 詳細版Markdownの最終形 | `news_briefing.py` の `_render_detailed_markdown()` |
| LINE長文分割 | `line_sender.py` の `split_line_message()` |

GitHub上で直接 `news_briefing.py` を編集した場合は、ローカルPCのGitHub Desktopや `git pull` で最新状態を取り込んでから、READMEや追加修正を行ってください。
