...
- AIモデル、AIエージェント、開発者ツール、AI検索、OS・端末AI、AI半導体、主要テック企業などの重要ニュースを集めます。
...
- LINE短縮版と詳細版は、同じ重要ニュースリストを同じ順序で使います。詳細版だけニュースが増えたり、LINE版と重要度・信頼度・URLがずれたりしないようにしています。
...
- GitHub: プログラムを置く場所です。
- GitHub Actions: ニュース生成プログラムを実行する機能です。
- GitHub Pages: `docs` フォルダのHTMLをWebページとして公開する機能です。
- LINE Messaging API: LINE公式アカウントから自分のLINEへメッセージを送る機能です。

流れは次の通りです。

1. Google Apps Scriptが毎朝7:07 JSTごろにGitHub Actionsを起動します。
2. 念のため、Google Apps Scriptが7:37 JSTごろにもバックアップ起動します。
3. `main.py` がOpenAI APIでニュースを収集し、共通の重要ニュースリストを作ります。
4. その同じリストからLINE短縮版と詳細版Markdownを作ります。
5. `docs/YYYY-MM-DD.html` と `docs/latest.html` を作ります。
6. LINE Messaging APIで短縮版を送ります。
7. 生成したHTMLをGitHubへcommit/pushします。
8. 同じ日付の送信済みマーカーがある場合は、バックアップ起動では二重送信せずに終了します。
...
5. `scheduled_dispatch` が表示された場合、通常の手動確認ではチェックを入れません。
6. 緑色の `Run workflow` ボタンをクリックします。
7. 実行中の行をクリックするとログが見られます。
...
このアプリの定期起動は、Google Apps Scriptの時間主導トリガーで行います。GAS側の `dispatchDailyAiNewsWorkflow` がGitHub Actionsの `workflow_dispatch` APIを呼び出し、同じ `daily_ai_news` workflowを起動します。

GAS側には次の2つの時間ベーストリガーを作ります。

- 7:07 JSTごろ
- 7:37 JSTごろ

以前はGitHub Actionsの `schedule` cronも使っていましたが、GitHub側の混雑で朝のcronが何時間も遅れて夜に実行されることがありました。そのため、現在はGitHub Actionsの `schedule` は使わず、GASからの `workflow_dispatch` を正式な定期起動ルートにしています。

バックアップ実行で同じニュースが2回LINEに届かないように、GASからの定期実行扱いの `workflow_dispatch` で、通常のニュース生成とLINE送信が完全成功した場合だけ `docs/.daily_ai_news_sent_YYYY-MM-DD` という送信済みマーカーを作ります。7:37 JSTのバックアップ実行時にこのマーカーがあれば、ニュース生成もLINE送信もせずに正常終了します。通常の手動実行は確認用なので、このマーカーがあっても従来通り実行できます。

次の場合は送信済みマーカーを作らず、7:37 JSTのバックアップ実行で再試行します。

- OpenAI APIやJSON解析に失敗し、管理者だけにエラー通知した場合
- `APPROVED_USERS_ENDPOINT` から承認済みユーザー一覧を取得できず、管理者だけにエラー通知した場合
- 複数ユーザーのうち一部だけLINE送信に失敗した場合

部分成功時は、すでに届いた人にもバックアップ実行で再送される可能性があります。このアプリでは、二重送信を完全に避けることよりも、その日の配信をできるだけ成功させることを優先しています。
...
OpenAI APIのレート制限やJSON/APIエラーで通常ニュースを生成できない場合、このアプリは承認済みユーザー全員への配信を中止し、`LINE_TO_ID` の管理者だけにエラー通知します。GitHub Actionsは失敗扱いになるため、7:37 JSTのバックアップ実行や次回実行で再試行できます。
...
1. Apps Scriptを開きます。
2. 左メニューの `トリガー` をクリックします。
3. `dispatchDailyAiNewsWorkflow` の時間ベーストリガーを削除します。

GitHub Actions側は `workflow_dispatch` のみ残しているため、GASトリガーを止めると自動配信は止まり、手動実行だけが残ります。
...
- GitHub Actions: GASから呼び出されたときにニュースを作り、詳細版HTMLを `docs` ディレクトリへ保存し、LINEへ送ります。7:37 JSTのバックアップ実行では、送信済みマーカーがある場合は二重送信しません。
...
### 21-8. GASからGitHub Actionsを起動する設定
...
### 21-11. GitHub Actionsの手動実行で確認する

1. GitHubのリポジトリ画面を開きます。
2. `Actions` タブをクリックします。
3. 左側の `daily_ai_news` をクリックします。
4. `Run workflow` をクリックします。
5. `scheduled_dispatch` は通常OFFのままにします。
6. 実行ログを開きます。
...
### 21-13. GAS Webhookが動かない場合の確認ポイント
...
- `GITHUB_DISPATCH_TOKEN` をREADMEやコードに書かないでください。
...
GASからGitHub Actionsを起動している場合は、Apps Scriptの左メニュー `トリガー` から `dispatchDailyAiNewsWorkflow` のトリガーも削除してください。またはApps Scriptで `deleteDailyAiNewsWorkflowTriggers` を実行します。
