...
8. 22:37 UTCにもバックアップ実行しますが、同じ日付の送信済みマーカーがある場合は二重送信せずに終了します。
...
さらに、GitHub Actionsのscheduleだけに依存しないため、Google Apps Scriptから `workflow_dispatch` を呼んでGitHub Actionsを起動する方式も用意しています。GAS側の時間主導トリガーで7:07 JSTと7:37 JSTに `dispatchDailyAiNewsWorkflow` を実行すると、GitHubの手動実行API経由で同じ `daily_ai_news` workflowが起動します。

バックアップ実行で同じニュースが2回LINEに届かないように、schedule実行、またはGASからの定期実行扱いの `workflow_dispatch` で、通常のニュース生成とLINE送信が完全成功した場合だけ `docs/.daily_ai_news_sent_YYYY-MM-DD` という送信済みマーカーを作ります。7:37 JSTのバックアップ実行時にこのマーカーがあれば、ニュース生成もLINE送信もせずに正常終了します。通常の手動実行は確認用なので、このマーカーがあっても従来通り実行できます。
...
GitHub Actionsのscheduleは、GitHub側の混雑で遅れたり実行されないことがあります。安定運用したい場合は、GASの時間主導トリガーでGitHub Actionsを起動します。
...
この2つのトリガーは、毎日7:07 JSTごろと7:37 JSTごろにGitHub Actionsを起動します。GASの時間主導トリガーも完全に秒単位ぴったりではありませんが、GitHub Actionsのscheduleだけに依存するより運用しやすくなります。
