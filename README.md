...
バックアップ実行で同じニュースが2回LINEに届かないように、schedule実行で通常のニュース生成とLINE送信が完全成功した場合だけ `docs/.daily_ai_news_sent_YYYY-MM-DD` という送信済みマーカーを作ります。7:37 JSTのバックアップ実行時にこのマーカーがあれば、ニュース生成もLINE送信もせずに正常終了します。手動実行は確認用なので、このマーカーがあっても従来通り実行できます。

次の場合は送信済みマーカーを作らず、7:37 JSTのバックアップ実行で再試行します。

- OpenAI APIやJSON解析に失敗し、フォールバック通知だけを送った場合
