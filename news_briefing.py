...
- 同じ日に複数回実行すると、新しい記事の出現で結果が変わり得る。その場合は実行時点で取得できる最新記事を優先し、全体所感でその可能性に触れる。
...
3. `news_items` は必ず Apple / Google / Microsoft / Meta / Amazon / Other AI のカテゴリ順で並べる。ここでいう `Meta / Amazon` は1つのカテゴリであり、MetaとAmazonのニュースをまとめる。同じカテゴリ内では重要度、新しさ、信頼性、実用性、技術的興味の順で並べる。
4. LINE短縮版 `line_message` も詳細版 `detailed_markdown` も、必ず Apple / Google / Microsoft / Meta / Amazon / Other AI / 今日のMust Read / 深掘り候補 / 全体所感 の順番で作る。ここでも `Meta / Amazon` は1つのカテゴリとして扱う。
5. LINE短縮版と詳細版で、カテゴリ、カテゴリ内のニュース順、タイトル、重要度、信頼度、URLを一致させる。
6. 各カテゴリに過去24時間以内の条件に合う主要記事がない場合は、古いニュースで埋めず「過去24時間以内に条件に合う主要ニュースはありません」と短く書く。
7. ニュースが少ない場合は水増しせず、`news_items` を3本未満にしてよい。その場合は `editor_note` とLINEの全体所感に「本日は条件に合う重要ニュースが少なめです」と書く。
8. `selection_basis` は、添付例と同じく「対象期間：」「ソース方針：」「有料記事の扱い：」「除外：」「今日の上位ニュースは、」の5件をこの順番で書く。

news_items の各項目:
- category: 必ず「Apple」「Google」「Microsoft」「Meta / Amazon」「Other AI」のいずれかにする。MetaとAmazonは「Meta / Amazon」にまとめる。OpenAI、Anthropic、NVIDIA、AI半導体、AIインフラ、AI規制、その他企業は「Other AI」にまとめる。
...
- user_meaning: 個人の関心、技術トレンド、製品体験、購入判断、主要企業動向の理解として何を意味するかを書く。
...
editor_note:
- 詳細版の「全体所感」に入れる文章として、今日のニュース全体から見える流れを2〜5文で書く。
- ニュースが少ない日、噂が多い日、公式発表が少ない日などはここに明記する。
- 特に書くことがなければ「特になし」と書く。
...
detailed_markdown:
必ず次の固定テンプレートで作る。添付例と同じカテゴリ順・見出し順にし、LINE短縮版と同じカテゴリ順、カテゴリ内順序、タイトル、重要度、信頼度、URLを使う。

# AIニュース詳細ブリーフィング

## Selection basis / 選定基準
- 対象期間：{{selection_basis内で24時間枠を説明}}
- ソース方針：{{selection_basis内でソース方針を説明}}
- 有料記事の扱い：{{selection_basis内で有料記事の扱いを説明}}
- 除外：{{selection_basis内で除外条件を説明}}
- 今日の上位ニュース：{{selection_basis内でトップニュースを説明}}
...
信頼度 / ソース種別：{{news_items[0].reliability}}
...
選定理由
{{news_items[0].selection_reason}}

中長期的な意味
{{news_items[0].long_term_significance}}

あなたにとっての意味
{{news_items[0].user_meaning}}

解釈・評価
{{news_items[0].analysis}}

次に起きそうなこと
{{news_items[0].outlook}}

ソースURL
{{news_items[0].source_url}}
...
重要な品質ルール:
- `news_items` の順序を、Apple / Google / Microsoft / Meta / Amazon / Other AI のカテゴリ順にする。
- LINE短縮版と詳細版は、必ず同じカテゴリ順、同じカテゴリ内順序にする。
- 詳細版の見出し番号はカテゴリ内の連番にする。LINE短縮版の各カテゴリ内番号と一致させる。
...
def _briefing_from_dict(
    ...
    *,
    ...
    if not selection_basis:
        selection_basis = [
            "対象期間：7:00 JSTのブリーフィング時点から過去24時間以内に公開された記事・公式発表だけを対象にしました。",
            "ソース方針：日本語記事を優先しつつ、公式発表や海外主要メディアの方が一次性・詳細性・速報性で優れる場合は採用します。",
            "有料記事の扱い：見出しだけでは採用せず、本文・スニペット・公式発表・信頼できる補強情報で内容確認できる場合だけ扱います。",
            "除外：株価、決算、投資家向け、見出しだけで中身を確認できない記事は除外しました。",
            "今日の上位ニュースは、ユーザー影響、プラットフォーム重要度、新しさ、信頼性、実用性、技術的興味の順に評価しました。",
        ]
...
def _normalize_news_items(raw_items: Any) -> list[dict[str, Any]]:
    ...
    return sorted(items, key=lambda item: category_rank.get(item["category"], len(CATEGORY_ORDER)))
...
def _render_detailed_markdown(
    *,
    ...
    lines: list[str] = [
        "# AIニュース詳細ブリーフィング",
        "",
        f"配信基準時刻：{_detail_title_date(date_jst)}",
        "",
        "## Selection basis / 選定基準",
    ]
    ...
    lines.extend(["", "## 深掘り候補"])
    lines.extend(f"- {item}" for item in deep_dive_candidates)
    lines.extend(["", "## 全体所感", editor_note or "特になし", ""])
    return "\n".join(lines).strip() + "\n"
...
def _fallback_briefing(
    *,
    ...
    detailed_markdown = f"""# AIニュース詳細ブリーフィング

配信基準時刻：{_detail_title_date(date_jst)}
